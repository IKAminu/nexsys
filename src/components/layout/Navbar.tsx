...
export default function Navbar() {
  ...
  return (
    <>
          ...
          >
              ...
              {siteConfig.nav.map((item) => (
                <li key={item.href}>
                  <Link
                    ...
                    style={{
                      color: isActive(item.href) ? "#F5F7FA" : "#8BA3BC",
                    }}
            ...
            >
              <span
                ...
                style={{
                  transform: menuOpen ? "rotate(45deg) translate(2px, 5px)" : "",
                }}
              />
              <span
      ...
      >
        <div className="flex items-center h-16 px-6 border-b border-[#1E3048]">
          <NexoraLogo size="md" />
        </div>
        <nav className="flex-1 flex flex-col justify-center px-8" aria-label="Mobile navigation">
          <ul className="space-y-2" role="list">
            {siteConfig.nav.map((item, i) => (
              <li key={item.href}>
                <Link
                  to={item.href}
                  className="flex items-center justify-between py-4 text-2xl font-semibold border-b border-[#1E3048] transition-colors"
                  style={{
                    color: isActive(item.href) ? "#F5F7FA" : "#8BA3BC",
          ...
          <div className="mt-8 text-xs" style={{ color: "#1E3048" }}>
            <p style={{ color: "#8BA3BC" }}>{siteConfig.contact.email}</p>
            <p style={{ color: "#8BA3BC" }}>{siteConfig.contact.phone}</p>
          </div>
        </nav>
      </div>
    </>
