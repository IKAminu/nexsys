  ...
  return (
    <div className="relative w-full" style={{ aspectRatio: "1 / 1", maxWidth: 580 }} aria-hidden="true">
      <div
        ...
        style={{ background: "radial-gradient(ellipse 70% 70% at 50% 50%, rgba(23, 105, 255, 0.06) 0%, transparent 70%)" }}
      />
      ...
      <svg viewBox="0 0 580 550" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative w-full h-full">
        ...
        {[[0, 14], [0, 5], [0, 15], [2, 0]].map(([a, b], i) => {
          ...
          return <circle key={`travel-${i}`} r="2" fill="#21D4FD" opacity="0.8"><animateMotion dur={`${2.5 + i * 0.8}s`} repeatCount="indefinite" begin={`${i * 0.6}s`}><mpath href={`#path-${i}`} /></animateMotion><animate attributeName="opacity" values="0;0.9;0" dur={`${2.5 + i * 0.8}s`} repeatCount="indefinite" begin={`${i * 0.6}s`} /></circle>
        })}
        ...
        <g filter="url(#glow)">
          ...
          <circle cx={290} cy={275} r={28} fill="#07111F" stroke="#1769FF" strokeWidth="1.5" />
          <circle cx={290} cy={275} r={22} fill="#0D1B2A" stroke="#1769FF" strokeWidth="0.75" strokeOpacity="0.4" />
          <g transform="translate(278, 263)"><path d="M0 0v24h5V8l12 16h5V0h-5v16L5 0H0z" fill="#1769FF" /><line x1="5" y1="0" x2="17" y2="24" stroke="#21D4FD" strokeWidth="1" opacity="0.8" /></g>
          <circle cx={290} cy={275} r={28} fill="none" stroke="#1769FF" strokeWidth="1"><animate attributeName="r" values="28;42;28" dur="3s" repeatCount="indefinite" /><animate attributeName="stroke-opacity" values="0.6;0;0.6" dur="3s" repeatCount="indefinite" /></circle>
        </g>
...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden" style={{ background: "#07111F" }} aria-labelledby="hero-heading">
      {/* World background — behind the entire hero, including text and network animation */}
      <img
        src="/nexsys/assets/world-background.svg"
        alt=""
